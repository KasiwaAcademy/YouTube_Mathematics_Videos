...
class Thumbnail(Scene):
    def construct(self):
        ...
        # Title text
        title = Text(
            "Coordinate Geometry", font="Roboto", weight=BOLD, color=YELLOW
        ).scale(1.5).to_edge(UP)

        subtitle = Text(
            "Finding Gradient of a Line", font="Roboto", weight=BOLD, color=WHITE
        ).scale(1.5).next_to(title, DOWN, buff=0.3)

        # Quadratic Formula
        formula = MathTex(
            r"m = \frac{y2 - y1}{x2 - x1}", color=WHITE
